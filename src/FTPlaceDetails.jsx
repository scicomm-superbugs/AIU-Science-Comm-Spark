...
export default function FTPlaceDetails() {
  ...
  const [judgeDocs, setJudgeDocs] = useState([]);
  ...
  const isStaff = userRole === 'admin' || userRole === 'master' || userRole === 'judge' || userRole === 'faculty';
    ...
    if (places && registrations) {
      ...
      if (found) {
        if (!isStaff && found.isVisible === false) {
          const isRegistered = registrations.some(r => r.placeId === placeId && r.competitorId === userId);
          if (!isRegistered) {
            setPlace(null);
            setLoading(false);
            return;
  ...
  useEffect(() => {
    (async () => {
      const ids = place?.judgeIds || (place?.judgeId ? [place.judgeId] : []);
      if (ids.length > 0) {
        const docs = [];
        for (const id of ids) {
  ...
  }, [place?.judgeIds, place?.judgeId]);

  const myRegistrations = useMemo(() => {
    if (!registrations) return [];
    return registrations.filter(r => r.placeId === placeId && r.competitorId === userId);
  ...
  const handleRemoveCompetitor = async (regId) => {
    if (!window.confirm('Are you sure you want to remove this competitor from this place?')) return;
    try {
      await db.ft_registrations.delete(regId);
      setToast({ type: 'success', msg: 'Competitor removed successfully!' });
    } catch (err) {
      setToast({ type: 'error', msg: 'Failed to remove competitor: ' + err.message });
    }
  ...
  const handleRegister = async () => {
    ...
    if (place?.hasPrograms) {
      if (!selectedProgramId) {
        setToast({ type: 'error', msg: 'Please select a competition program.' });
        setTimeout(() => setToast(null), 3000);
        return;
      }
      if (chosenProgram?.waves && chosenProgram.waves.length > 0 && !selectedWaveId) {
        setToast({ type: 'error', msg: 'Please select a competition wave/duration for your selected program.' });
        setTimeout(() => setToast(null), 3000);
        return;
      }
      ...
      try {
        await db.ft_registrations.add({
          ...
          competitorName: meDoc?.name || '',
          competitorDepartment: meDoc?.department || '',
          competitorUniversityId: meDoc?.universityId || '',
          competitorPhone: meDoc?.phone || '',
        ...
        await db.ft_notifications.add({
          ...
          message: `${meDoc?.name || 'A competitor'} requested to register for ${place.name}${chosenWave ? ` (${chosenWave.name})` : ''}`,
    ...
    if (place?.waves && place.waves.length > 0 && !selectedWaveId) {
      setToast({ type: 'error', msg: 'Please select a competition wave/duration.' });
      setTimeout(() => setToast(null), 3000);
      return;
    }
  ...
  const handleUnregister = async (targetReg) => {
    ...
    if (targetReg.status === 'completed') {
      setToast({ type: 'error', msg: 'Cannot unregister from a completed competition.' });
      setTimeout(() => setToast(null), 3000);
      return;
    }
    if (needsApproval) {
      try {
        ...
        await db.ft_notifications.add({
          ...
          message: `${meDoc?.name || 'A competitor'} requested to cancel registration for ${place.name}`,
    ...
    try {
      if (requiresApproval) {
        ...
        await db.ft_notifications.add({
          ...
          message: `${meDoc?.name || 'A competitor'} requested to change registration for ${place.name}`,
          type: 'change_request',
          status: 'unread',
          targetRoles: ['admin', 'master', 'faculty'],
          targetUserId: null,
  ...
  if (!place) {
    return (
      <div className="ft-empty">
        ...
        <div className="ft-empty-text">This competition place doesn't exist or has been removed.</div>
        ...
        <div className="ft-place-detail-grid">
          ...
          <div>
            <div className="ft-card" style={{ marginBottom: '1.5rem' }}>
              <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: '1.2rem', fontWeight: 700, marginBottom: '1rem' }}>About This Competition Place</h3>
              <p style={{ color: 'var(--ft-text-secondary)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>
                {place.description || 'No description has been provided for this competition place.'}
              </p>
            </div>
            ...
            {place.thesis && (
              <div className="ft-card" style={{ marginBottom: '1.5rem' }}>
                <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: '1.2rem', fontWeight: 700, marginBottom: '1rem' }}>📄 Competition Topics</h3>
                <p style={{ color: 'var(--ft-text-secondary)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>
                  {place.thesis}
                </p>
              </div>
          ...
          <div className="ft-place-detail-sticky">
            <div className="ft-card">
              ...
              <div style={{ padding: '0.5rem 0' }}>
                ...
                {judgeDocs.length > 0 && (
                  <div className="ft-place-info-row" style={{ alignItems: 'flex-start', flexWrap: 'wrap' }}>
                    ...
                    <span className="ft-place-info-label">Judge{judgeDocs.length > 1 ? 's' : ''}</span>
              ...
              {/* Current Competitor Registrations (if any) */}
                      ...
                      <div key={reg.id} style={{ border: '1.5px solid var(--ft-border)', borderRadius: 'var(--ft-radius)', padding: '1rem', background: 'var(--ft-bg-card)', display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
                          ...
                          <span className={`ft-badge ft-badge-${reg.status}`} style={{ fontSize: '0.74rem', padding: '0.15rem 0.5rem', height: 'auto' }}>
                            ...
                            {reg.status === 'active' && '🔵 In Competition'}
              ...
              {(allowSelfRegister && (place.hasPrograms || !myRegistrations.some(r => r.status !== 'failed'))) ? (
                  ...
                  {paymentRequired && (
                    <div 
                      ...
                      <p style={{ fontSize: '0.8rem', color: 'var(--ft-text-secondary)', margin: 0, lineHeight: 1.5 }}>
                        This competition program or wave requires registration payment. Please click the button below to pay, and then upload your payment receipt to complete your registration.
                      </p>
                      {activePaymentLink ? (
                        <a 
                ...
                <div style={{ padding: '1rem 0 0', borderTop: '1px solid var(--ft-border-light)', fontSize: '0.85rem', color: 'var(--ft-text-secondary)', lineHeight: 1.6 }}>
                  <div style={{ background: 'var(--ft-bg-input)', border: '1px dashed var(--ft-border)', padding: '1rem', borderRadius: 'var(--ft-radius-md)' }}>
                    📢 <strong>Registration Notice:</strong> Enrollment is managed solely by university administrators via manual competitor imports. If you are pre-assigned to this place, your account will be linked automatically.
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="ft-modal-overlay">
          <div className="ft-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '400px' }}>
            <div className="ft-modal-body" style={{ textAlign: 'center', padding: '2rem' }}>
              ...
              <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: '1.2rem', fontWeight: 700, marginBottom: '0.5rem' }}>Remove Competitor?</h3>
              <p style={{ fontSize: '0.88rem', color: 'var(--ft-text-secondary)', marginBottom: '1.5rem' }}>
                Are you sure you want to remove this competitor from this competition place? This will cancel their registration.
              </p>
