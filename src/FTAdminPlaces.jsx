...
const ensureTestCompetitorsForPlace = async (place, registrations) => {
    ...
    if (list.length > 1) {
      ...
      for (const r of toDelete) {
        ...
        try {
          ...
          console.error("Failed to delete duplicate test competitor:", err);
        } finally {
  ...
  // 2. Add missing test competitor registrations
    ...
    if (!exists) {
      ...
      try {
        await db.ft_registrations.add({
          competitorId: 'test_competitor_fallback',
          placeId: place.id,
          ...
          competitorDepartment: place.department || 'Test Dept',
          competitorUniversityId: 'TEST-0000',
          competitorEmail: 'test.competitor@aiu.edu.eg',
          waveId: spec.waveId || null,
          waveName: spec.waveName || null,
          isTest: true
        });
      } catch (err) {
        console.error("Failed to add test competitor:", err);
      } finally {
  ...
  // 3. Clean up obsolete test competitor registrations (e.g. if wave or program is removed)
  ...
  for (const r of obsoleteRegs) {
    ...
    try {
      ...
      console.error("Failed to delete obsolete test competitor:", err);
    } finally {
      setTimeout(() => inFlightAdditions.delete(delKey), 2000);
    }
  }
...
export default function FTAdminPlaces() {
  ...
  const [enrollCompetitorId, setEnrollCompetitorId] = useState('');
  const [enrollCompetitorName, setEnrollCompetitorName] = useState('');
  const [enrollCompetitorEmail, setEnrollCompetitorEmail] = useState('');
  const [enrollCompetitorDept, setEnrollCompetitorDept] = useState('');
  ...
  const handleOpenEnrolledModal = (placeId) => {
    ...
    setEnrollCompetitorId('');
    setEnrollCompetitorName('');
    setEnrollCompetitorEmail('');
    setEnrollCompetitorDept('');
    setEnrollWaveId('');
    setEnrollProgramId('');
  ...
  const handleManualEnroll = async (place) => {
    if (!enrollCompetitorId.trim()) {
      setToast({ type: 'error', msg: 'Please enter a Competitor ID.' });
    ...
    const isAlreadyRegistered = registrations?.some(r => 
      r.placeId === place.id && 
      r.competitorUniversityId.trim() === enrollCompetitorId.trim() &&
      r.status !== 'failed'
    );
    if (isAlreadyRegistered) {
      setToast({ type: 'error', msg: 'This competitor is already enrolled in this competition place.' });
      setTimeout(() => setToast(null), 3000);
      return;
    }

    try {
      const matchedSci = scientists.find(s => s.universityId === enrollCompetitorId.trim());
      ...
      await db.ft_registrations.add({
        competitorId: matchedSci?.id || null,
        placeId: place.id,
        placeName: place.name,
        ...
        status: 'active',
        registeredAt: new Date().toISOString(),
        competitorName: matchedSci?.name || enrollCompetitorName.trim() || 'Imported Trainee',
        competitorDepartment: matchedSci?.department || enrollCompetitorDept.trim() || '',
        competitorUniversityId: enrollCompetitorId.trim(),
        competitorEmail: matchedSci?.email || enrollCompetitorEmail.trim() || '',
        waveId: enrollWaveId || null,
        waveName: waveName || null
      });

      setToast({ type: 'success', msg: 'Competitor enrolled successfully!' });
      setEnrollCompetitorId('');
      setEnrollCompetitorName('');
      setEnrollCompetitorEmail('');
      setEnrollCompetitorDept('');
  ...
  const [form, setForm] = useState({
    ...
    creditHours: '', department: '', capacity: '', judgeId: '', judgeIds: [], image: '',
  ...
  const downloadImportTemplate = (place) => {
    ...
    // The test competitor must be registered for all waves/programs in the template to show how they are formatted
    if (hasMultiplePrograms) {
      place.programs.forEach(prog => {
        if (prog.waves && prog.waves.length > 0) {
          prog.waves.forEach(w => {
  ...
  const handleCSVImport = async (e) => {
    ...
    reader.onload = async (evt) => {
      ...
      try {
        ...
        const nameIdx = headers.findIndex(h => h.includes('name') || h.includes('competitor'));
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const existingReg = registrations?.find(r => 
            ...
            (r.competitorUniversityId === rowId || (rowEmail && r.competitorEmail?.toLowerCase() === rowEmail.toLowerCase()))
          );
          ...
          if (scientist) {
            await db.ft_registrations.add({
              ...
              status: 'active',
              registeredAt: new Date().toISOString(),
              competitorName: scientist.name || rowName,
              competitorDepartment: scientist.department || '',
              competitorUniversityId: scientist.universityId || rowId,
              competitorEmail: scientist.email || rowEmail,
              waveId: matchedWave.id,
            ...
            await db.ft_registrations.add({
              ...
              competitorDepartment: '',
              competitorUniversityId: rowId,
              competitorEmail: rowEmail,
              waveId: matchedWave.id,
              waveName: `${matchedWave.name} (${matchedWave.duration})`
            });
          }
        ...
        setToast({ type: 'success', msg: `Successfully imported ${imported} competitors! (${errors} errors)` });
      } catch (err) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const list = allScientists.filter(u => ['judge', 'admin', 'master'].includes(u.role) && u.accountStatus === 'active');
        setJudges(list);
      } catch (err) {
  ...
  // Ensure test competitors for all places
  useEffect(() => {
    if (places && registrations) {
      (async () => {
        for (const place of places) {
          await ensureTestCompetitorsForPlace(place, registrations);
        }
      })();
    }
  ...
  const openEditModal = (place) => {
    ...
    setForm({
      ...
      judgeIds: place.judgeIds || (place.judgeId ? [place.judgeId] : []),
      image: place.image || '',
      waves: place.waves || [],
      ...
      if (resetReq) {
        ...
        if (!snap.empty) {
          ...
          await db.ft_notifications.add({
            ...
            status: 'unread',
            targetRoles: ['competitor', 'user', 'judge'],
  ...
  const handleApproveChangeRequest = async (reg) => {
    ...
    try {
      if (changeRequest.type === 'cancel') {
        ...
        setToast({ type: 'success', msg: `Approved cancellation request for ${reg.competitorName}.` });
        ...
        setToast({ type: 'success', msg: `Approved change request for ${reg.competitorName}.` });
      }
    } catch (err) {
      setToast({ type: 'error', msg: 'Failed to approve: ' + err.message });
    }
  ...
  const handleRejectChangeRequest = async (reg) => {
    try {
      ...
      setToast({ type: 'info', msg: `Rejected request for ${reg.competitorName}.` });
    } catch (err) {
      setToast({ type: 'error', msg: 'Failed to reject: ' + err.message });
    }
    setTimeout(() => setToast(null), 3000);
  ...
  const exportPlaceCSV = (place) => {
    ...
    let headers = ['Competitor Name', 'University ID', 'Competitor Phone', 'Department', 'Selected Wave', 'Wave Start Date', 'Wave End Date', 'Wave Deadline', 'Payment Required', 'Payment Receipt', 'Registration Status', 'Registered Date'];
    if (hasMultiplePrograms) {
      headers = ['Competitor Name', 'University ID', 'Competitor Phone', 'Department', 'Program', 'Selected Wave', 'Wave Start Date', 'Wave End Date', 'Wave Deadline', 'Payment Required', 'Payment Receipt', 'Registration Status', 'Registered Date'];
    }
      ...
      <div className="ft-animate-in">
        <div className="ft-page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            ...
            <p className="ft-page-subtitle">Add, edit, or remove competition places.</p>
          </div>
        ...
        <div className="ft-card ft-animate-in" style={{ marginBottom: '2.5rem', padding: '1.25rem' }}>
          <div className="ft-admin-hub-header">
            ...
            <div className="ft-admin-hub-tabs">
              {[
                ...
                { id: 'notes', label: '📋 Judge Notes', count: evaluations.length }
              ].map(t => (
                <button
                  key={t.id}
                  type="button"
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                <p style={{ fontSize: '0.82rem', color: 'var(--ft-text-muted)', margin: 0 }}>
                  Approve or reject competitors requesting to register for competition spots.
                </p>
                      ...
                      <div key={reg.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--ft-bg-input)', padding: '0.75rem 1rem', borderRadius: 'var(--ft-radius-sm)', border: '1.5px solid var(--ft-border)', flexWrap: 'wrap', gap: '1rem' }}>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                            <span style={{ fontWeight: 700, fontSize: '0.9rem' }}>{reg.competitorName}</span>
                            {reg.competitorUniversityId && (
          ...
          {activeTab === 'changes' && (
            <div>
              <p style={{ fontSize: '0.82rem', color: 'var(--ft-text-muted)', marginBottom: '0.75rem' }}>
                Manage competitors requesting to switch waves/programs, or fully cancel their competition spots.
              </p>
          ...
          {activeTab === 'resets' && (
            <div>
              <p style={{ fontSize: '0.82rem', color: 'var(--ft-text-muted)', marginBottom: '0.75rem' }}>
                Approve password reset requests submitted by locked out competitors or judges.
              </p>
          ...
          {/* TAB 5: Judge evaluation notes */}
          {activeTab === 'notes' && (
            <div>
              <p style={{ fontSize: '0.82rem', color: 'var(--ft-text-muted)', marginBottom: '0.75rem' }}>
                All evaluation notes, performance comments, and feedback written by judges for trainees.
              </p>
                  ...
                  {evaluations.map(ev => (
                    <div key={ev.id} style={{ background: 'var(--ft-bg-input)', padding: '0.85rem 1rem', borderRadius: 'var(--ft-radius-sm)', border: '1.5px solid var(--ft-border)', marginBottom: '0.5rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.35rem', borderBottom: '1px dashed var(--ft-border-light)', paddingBottom: '0.25rem' }}>
                        <span style={{ fontWeight: 700, fontSize: '0.85rem' }}>👨‍🎓 Trainee: {ev.competitorName} ({ev.competitorUniversityId || 'No ID'})</span>
                      ...
                      <div style={{ fontSize: '0.72rem', color: 'var(--ft-text-muted)', marginTop: '0.35rem', textAlign: 'right' }}>
                        Evaluated by: <strong>{ev.evaluatedBy || 'Judge'}</strong> on {ev.createdAt ? new Date(ev.createdAt).toLocaleDateString() : '—'}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {filteredPlaces.length === 0 ? (
          <div className="ft-empty">
            ...
            <div className="ft-empty-title">No Competition Places</div>
            <div className="ft-empty-text">Get started by adding your first competition place.</div>
            ...
            <div className="ft-table-wrapper" onScroll={(e) => { if (e.target.scrollLeft > 10) setHasSwipedTable(true); }}>
              ...
              <tbody>
                {filteredPlaces.map(place => {
                  const assignedJudges = judges.filter(t => place.judgeIds?.includes(t.id) || t.id === place.judgeId);
                      ...
                      <td style={{ position: 'relative' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                          ...
                          <div>
                            ...
                            {/* Toggle seats breakdown & Enrolled Competitors */}
                      ...
                      <td style={{ fontSize: '0.82rem' }}>
                        {assignedJudges.length > 0 ? (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                            {assignedJudges.map(t => <div key={t.id} style={{ whiteSpace: 'nowrap' }}>{t.name}</div>)}
                          </div>
                        ) : '—'}
                      </td>
                      <td>
            ...
            <div className="ft-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '550px' }}>
              ...
              <div className="ft-modal-body" style={{ padding: '1.5rem' }}>
                <p style={{ color: 'var(--ft-text-secondary)', fontSize: '0.88rem', lineHeight: 1.6, margin: '0 0 1.5rem 0' }}>
                  Upload a CSV file to manually register competitors to <strong>{place.name}</strong>. The file must contain column headers for: <strong>University ID</strong> and <strong>Wave</strong> (both required).
                  {place.hasPrograms && place.programs && place.programs.length > 1 && (
                    <>
                      {' '}Also, since this place offers multiple competition programs, a <strong>Program</strong> column is <strong>required</strong>.
                    </>
                  )}
                  {' '}Columns for <strong>Name</strong> and <strong>Email</strong> are optional — competitor details will be auto-filled when their accounts are created and IDs match.
                </p>
            ...
            <form onSubmit={handleSubmit}>
              <div className="ft-modal-body">
                ...
                <div className="ft-input-group">
                  ...
                  <textarea className="ft-textarea" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Describe the competition place, what competitors will learn..." rows={3} />
                </div>

                <div className="ft-input-group">
                  <label className="ft-label">Competition Topics</label>
                  <textarea className="ft-textarea" value={form.thesis} onChange={e => setForm(f => ({ ...f, thesis: e.target.value }))} placeholder="Related competition topics..." rows={2} />
                </div>

                <div className="ft-input-group">
                  <label className="ft-label">Requirements</label>
                  <textarea className="ft-textarea" value={form.requirements} onChange={e => setForm(f => ({ ...f, requirements: e.target.value }))} placeholder="Prerequisites or requirements for competitors..." rows={2} />
                </div>

                <div className="ft-input-group">
                  <label className="ft-label">Assign Judges</label>
                    <div style={{
                      border: '1.5px solid var(--ft-border)',
                      ...
                      {judges.map(t => {
                        const isChecked = form.judgeIds?.includes(t.id);
                        return (
                          <label key={t.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', margin: 0, fontSize: '0.88rem', fontWeight: 500 }}>
                            <input
                              ...
                              onChange={(e) => {
                                if (e.target.checked) {
                                  setForm(f => ({ ...f, judgeIds: [...(f.judgeIds || []), t.id] }));
                                } else {
                                  setForm(f => ({ ...f, judgeIds: (f.judgeIds || []).filter(id => id !== t.id) }));
                                }
                              }}
                      ...
                      {judges.length === 0 && (
                        <div style={{ color: 'var(--ft-text-muted)', fontSize: '0.82rem', textAlign: 'center' }}>
                          No eligible judge accounts found.
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="ft-input-group" style={{ flexDirection: 'row', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem', marginBottom: '0.5rem' }}>
                  ...
                  <label htmlFor="isVisible-checkbox" className="ft-label" style={{ margin: 0, cursor: 'pointer', fontWeight: 600 }}>
                    Visible to Competitors (uncheck to hide place)
                  </label>
                </div>
                ...
                <div style={{ background: 'var(--ft-bg-card)', border: '1.5px solid var(--ft-border)', borderRadius: 'var(--ft-radius-sm)', padding: '0.85rem', marginBottom: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                 ...
                 <div className="ft-input-group" style={{ flexDirection: 'row', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem', marginBottom: '1.25rem' }}>
                   ...
                   <label htmlFor="hasPrograms-checkbox" className="ft-label" style={{ margin: 0, cursor: 'pointer', fontWeight: 600, color: 'var(--ft-primary)' }}>
                     Enable Multiple Competition Programs for this place
                   </label>
                 </div>
                 ...
                 {form.hasPrograms ? (
                   ...
                   <div style={{ margin: '1.5rem 0', borderTop: '1px solid var(--ft-border-light)', paddingTop: '1rem' }}>
                     <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                       <label className="ft-label" style={{ margin: 0, fontSize: '0.88rem', fontWeight: 700, color: 'var(--ft-primary)' }}>🎓 Competition Programs</label>
                           ...
                           <div key={prog.id} style={{ background: 'var(--ft-bg-input)', border: '1.5px solid var(--ft-border)', padding: '1.25rem', borderRadius: 'var(--ft-radius-md)', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                             ...
                             <div className="ft-input-group">
                               ...
                               <textarea
                                 ...
                                 placeholder="Details about this specific competition program..."
                                 rows={2}
                               />
                             </div>
                   ...
                   <div style={{ margin: '1.5rem 0', borderTop: '1px solid var(--ft-border-light)', paddingTop: '1rem' }}>
                     <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                       <label className="ft-label" style={{ margin: 0, fontSize: '0.85rem', fontWeight: 700, color: 'var(--ft-primary)' }}>🌊 Competition Waves / Durations</label>
      ...
      {showEnrolledModal && (() => {
        ...
        const filteredEnrolledRegs = enrolledRegs.filter(r => {
          const sci = scientists.find(s => 
            (r.competitorId && s.id === r.competitorId) ||
            (r.competitorUniversityId && s.universityId === r.competitorUniversityId) ||
            (r.competitorEmail && s.email?.toLowerCase() === r.competitorEmail.toLowerCase())
          );
          const dispName = sci?.name || r.competitorName || 'Imported Trainee (Pending Account)';
          const dispId = sci?.universityId || r.competitorUniversityId || '';
        ...
        return (
          <div className="ft-modal-overlay">
            <div className="ft-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '900px', width: '95%' }}>
              <div className="ft-modal-header">
                <h3 className="ft-modal-title" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  👥 Enrolled Competitors ({enrolledRegs.length})
                </h3>
                <button 
                  type="button" 
                  className="ft-btn ft-btn-ghost ft-btn-icon" 
              ...
              <div className="ft-modal-body" style={{ padding: '1.5rem', maxHeight: '450px', overflowY: 'auto' }}>
                ...
                <div style={{ marginBottom: '1.25rem', display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  <div className="ft-search-input-wrapper" style={{ flex: 1 }}>
                    ...
                    <input 
                      type="text" 
                      placeholder="Search by competitor name or ID..." 
                  ...
                  >
                    {showEnrollForm ? 'Cancel' : '➕ Enroll Competitor'}
                  </button>
                </div>
                  ...
                  <div className="ft-card" style={{ background: 'var(--ft-bg-input)', border: '1.5px solid var(--ft-border)', padding: '1rem', borderRadius: 'var(--ft-radius-lg)', marginBottom: '1.25rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '0.75rem' }}>
                      <div className="ft-input-group" style={{ margin: 0, position: 'relative' }}>
                        <label className="ft-label" style={{ fontSize: '0.75rem' }}>Competitor ID Number *</label>
                        <input 
                          type="text" 
                          className="ft-input" 
                          placeholder="e.g. 23100571" 
                          value={enrollCompetitorId}
                          onChange={e => {
                            const val = e.target.value;
                            setEnrollCompetitorId(val);
                            ...
                            if (exact) {
                              setEnrollCompetitorName(exact.name || '');
                              setEnrollCompetitorEmail(exact.email || '');
                              setEnrollCompetitorDept(exact.department || '');
                            }
                          }}
                        ...
                        {showSuggestions && (() => {
                          const query = enrollCompetitorId.trim().toLowerCase();
                            ...
                            <div className="ft-suggestions-list">
                              {matches.map(s => (
                                <div 
                                  ...
                                  onClick={() => {
                                    setEnrollCompetitorId(s.universityId || '');
                                    setEnrollCompetitorName(s.name || '');
                                    setEnrollCompetitorEmail(s.email || '');
                                    setEnrollCompetitorDept(s.department || '');
                                    setShowSuggestions(false);
                                  }}
                                >
                      ...
                      <div className="ft-input-group" style={{ margin: 0 }}>
                        <label className="ft-label" style={{ fontSize: '0.75rem' }}>Competitor Name (optional)</label>
                        <input 
                          type="text" 
                          className="ft-input" 
                          placeholder="e.g. John Doe" 
                          value={enrollCompetitorName}
                          onChange={e => setEnrollCompetitorName(e.target.value)}
                          style={{ padding: '0.5rem 0.75rem', fontSize: '0.82rem' }}
                        />
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '0.75rem' }}>
                      <div className="ft-input-group" style={{ margin: 0 }}>
                        <label className="ft-label" style={{ fontSize: '0.75rem' }}>Competitor Email (optional)</label>
                        <input 
                          type="email" 
                          className="ft-input" 
                          placeholder="e.g. competitor@aiu.edu.eg" 
                          value={enrollCompetitorEmail}
                          onChange={e => setEnrollCompetitorEmail(e.target.value)}
                          style={{ padding: '0.5rem 0.75rem', fontSize: '0.82rem' }}
                        />
                      </div>

                      <div className="ft-input-group" style={{ margin: 0 }}>
                        <label className="ft-label" style={{ fontSize: '0.75rem' }}>Competitor Department (optional)</label>
                        <input 
                          type="text" 
                          className="ft-input" 
                          placeholder="e.g. Biotechnology" 
                          value={enrollCompetitorDept}
                          onChange={e => setEnrollCompetitorDept(e.target.value)}
                          style={{ padding: '0.5rem 0.75rem', fontSize: '0.82rem' }}
                        />
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1rem' }}>
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="ft-input-group" style={{ margin: 0 }}>
                            <label className="ft-label" style={{ fontSize: '0.75rem' }}>Select Competition Wave</label>
                            <select
                              className="ft-select"
                              value={enrollWaveId}
                ...
                {enrolledRegs.length === 0 ? (
                  <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--ft-text-muted)', background: 'var(--ft-bg-input)', borderRadius: 'var(--ft-radius)', fontSize: '0.88rem' }}>
                    👥 No competitors currently enrolled in this competition place. Click "Enroll Competitor" above to manually add one!
                  </div>
                    ...
                    <table className="ft-table" style={{ fontSize: '0.85rem' }}>
                      ...
                      <tbody>
                        {filteredEnrolledRegs.map(reg => {
                          const sci = scientists.find(s => 
                            (reg.competitorId && s.id === reg.competitorId) ||
                            (reg.competitorUniversityId && s.universityId === reg.competitorUniversityId) ||
                            (reg.competitorEmail && s.email?.toLowerCase() === reg.competitorEmail.toLowerCase())
                          );
                          const dispName = sci?.name || reg.competitorName || 'Imported Trainee (Pending Account)';
                          const dispEmail = sci?.email || reg.competitorEmail || '';
                          const dispId = sci?.universityId || reg.competitorUniversityId || '—';
                          const dispDept = sci?.department || reg.competitorDepartment || '—';

                          return (
                            <tr key={reg.id}>
                              <td style={{ position: 'relative' }}>
                                {!hasSwipedEnrolledTable && <div className="ft-swipe-indicator" style={{ left: 'calc(100vw - 190px)', right: 'auto', zIndex: 1, pointerEvents: 'none' }}>👈 Swipe</div>}
                              ...
                              <td style={{ textAlign: 'center' }}>
                                <button 
                                  ...
                                  title="Remove Competitor Enrollment"
                                >
                                  <Trash2 size={14} />
                                </button>
                              </td>
            ...
            <div className="ft-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '400px' }}>
              <div className="ft-modal-body" style={{ textAlign: 'center', padding: '2.5rem 2rem 2rem' }}>
                ...
                <p style={{ fontSize: '0.88rem', color: 'var(--ft-text-secondary)', marginBottom: '1.75rem', lineHeight: 1.5 }}>
                  Are you sure you want to permanently remove trainee <strong style={{ color: 'var(--ft-text)' }}>{reg.competitorName || 'Imported Trainee'}</strong> (ID: {reg.competitorUniversityId || '—'}) from <strong style={{ color: 'var(--ft-text)' }}>{place?.name || 'this place'}</strong>? This action cannot be undone.
                </p>
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  ...
                  <button className="ft-btn ft-btn-danger" style={{ flex: 1 }} onClick={async () => {
                    ...
                    try {
                      await db.ft_registrations.delete(reg.id);
                      setToast({ type: 'success', msg: `Successfully removed enrollment for ${reg.competitorName || 'competitor'}.` });
                    } catch (err) {
          ...
          <div className="ft-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '400px' }}>
            <div className="ft-modal-body" style={{ textAlign: 'center', padding: '2rem' }}>
              ...
              <p style={{ color: 'var(--ft-text-muted)', fontSize: '0.88rem', marginBottom: '1.5rem' }}>
                This action cannot be undone. All competitor registrations for this place will remain in the system.
              </p>
