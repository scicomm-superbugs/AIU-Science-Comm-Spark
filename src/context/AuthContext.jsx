  ...
  const completeGoogleRegistration = async (googleData, extraData) => {
    ...
    if (targetAccount) {
      ...
      await db.scientists.update(targetAccount.id, {
        ...
        title: (extraData && extraData.title) ? extraData.title.trim() : (isMasterAdminEmail ? 'Teaching Assistant at Alamein International University' : (isJudge ? 'Judge' : 'Competitor')),
        role: role,
